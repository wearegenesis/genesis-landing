export default function GenesisBackground() {
  ...
  const STAR_COUNT = 60; // prueba: 60 / 80 / 110
  const MIN_SIZE = 3; // antes 1-2 -> ahora más visibles
  const MAX_SIZE = 7; // prueba: 5 / 6 / 7
  const OP_MIN = 0.08;
  const OP_MAX = 0.16;
  ...
  const stars = Array.from({ length: STAR_COUNT }).map((_, i) => {
    const top = 2 + rand() * 96; // evita bordes
    ...
    // Algunas un pelín más “brillantes”
    ...
    <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
      ...
      <style>{`
        ...
        @keyframes genesisTwinkle {
          0%   { opacity: 0.03; transform: translate(0,0) scale(0.95); }
          35%  { opacity: 0.22; transform: translate(1px,-1px) scale(1.15); }
          70%  { opacity: 0.08; transform: translate(-1px,1px) scale(1.0); }
          100% { opacity: 0.03; transform: translate(0,0) scale(0.95); }
        }
